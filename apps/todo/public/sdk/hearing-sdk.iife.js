...
`;function h(){if(document.getElementById("hearing-sdk-styles"))return;const e=document.createElement("style");e.id="hearing-sdk-styles",e.textContent=w,document.head.appendChild(e)}const b='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>';function x(e){h();const t=document.createElement("button");return t.className=`hearing-sdk-floating-button ${e.position} hearing-sdk-floating-button--pulse`,t.innerHTML=`${b}<span>${e.text}</span>`,t.setAttribute("aria-label",e.text),t.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),e.onClick()}),t}function v(){const e=document.querySelector(".hearing-sdk-floating-button");e&&e.remove()}function y(){h();const e=document.createElement("div");e.className="hearing-sdk-recording-indicator",e.id="hearing-sdk-recording-indicator";const t=document.createElement("div");t.className="hearing-sdk-recording-indicator__dot";const n=document.createElement("span");return n.textContent="REC",e.appendChild(t),e.appendChild(n),e}function I(){if(document.getElementById("hearing-sdk-recording-indicator"))return;const e=y();document.body.appendChild(e)}function E(){const e=document.getElementById("hearing-sdk-recording-indicator");e&&e.remove()}const T='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>';function H(e){h();const t=document.createElement("div");if(t.className=`hearing-sdk-task-panel ${e.position}`,e.todos.length===0)return t;const n=document.createElement("div");n.className="hearing-sdk-task-panel-header",n.textContent="Tasks to Complete",t.appendChild(n);const s=document.createElement("ul");return s.className="hearing-sdk-task-panel-list",e.todos.forEach(i=>{const o=document.createElement("li");o.className="hearing-sdk-task-panel-item",o.innerHTML=`
      <span class="hearing-sdk-task-panel-checkbox">${T}</span>
      <span class="hearing-sdk-task-panel-content">${i.content}</span>
    `,o.addEventListener("click",()=>{o.classList.toggle("completed")}),s.appendChild(o)}),t.appendChild(s),t}function C(){const e=document.querySelector(".hearing-sdk-task-panel");e&&e.remove()}class g{constructor(t){this.eventQueue=[],this.startTime=0,this.flushIntervalId=null,this.initialized=!1,this.returnUrl="",this.todos=[],this.clickHandler=null,this.scrollHandler=null,this.visibilityHandler=null,this.getElapsedMs=()=>Date.now()-this.startTime,this.addEvent=n=>{this.eventQueue.push(n),this.log("Event added:",n.eventType,this.eventQueue.length)},this.config={sessionId:t.sessionId,supabaseUrl:t.supabaseUrl,debug:t.debug??!1,flushInterval:t.flushInterval??5e3,buttonText:t.buttonText??"Return to Interview",buttonPosition:t.buttonPosition??"bottom-right"},this.apiClient=new p(this.config.supabaseUrl,this.config.debug)}log(...t){this.config.debug&&console.log("[HearingSDK]",...t)}async init(){if(this.initialized)return this.log("Already initialized"),!0;this.log("Initializing with sessionId:",this.config.sessionId);const t=await this.apiClient.getContext(this.config.sessionId);return t?(this.returnUrl=t.returnUrl,this.todos=t.todos||[],this.startTime=Date.now(),this.clickHandler=f(this.addEvent,this.getElapsedMs),this.scrollHandler=m(this.addEvent,this.getElapsedMs),this.visibilityHandler=k(this.addEvent,this.getElapsedMs),document.addEventListener("click",this.clickHandler,!0),window.addEventListener("scroll",this.scrollHandler,{passive:!0}),document.addEventListener("visibilitychange",this.visibilityHandler),this.addEvent({eventType:"page_view",pageUrl:window.location.href,pageTitle:document.title,viewportWidth:window.innerWidth,viewportHeight:window.innerHeight,elapsedMs:0,timestamp:new Date().toISOString()}),I(),this.showFloatingButton(),this.flushIntervalId=setInterval(()=>{this.flush()},this.config.flushInterval),window.addEventListener("beforeunload",()=>{this.flush()}),this.initialized=!0,this.log("Initialized successfully"),!0):(this.log("Failed to get context"),!1)}showFloatingButton(){if(this.todos.length>0){const n=H({todos:this.todos,position:this.config.buttonPosition});document.body.appendChild(n)}const t=x({text:this.config.buttonText,position:this.config.buttonPosition,onClick:()=>this.handleReturn()});document.body.appendChild(t)}async handleReturn(){this.log("Return button clicked");const t=await this.apiClient.markReturned(this.config.sessionId,window.location.href,this.getElapsedMs());await this.flush(),this.destroy();const n=(t==null?void 0:t.returnUrl)||this.returnUrl;n&&(window.location.href=n)}async flush(){if(this.eventQueue.length===0)return;const t=[...this.eventQueue];this.eventQueue=[],this.log("Flushing events:",t.length),await this.apiClient.ingestEvents(this.config.sessionId,t)}destroy(){this.log("Destroying tracker"),this.clickHandler&&document.removeEventListener("click",this.clickHandler,!0),this.scrollHandler&&window.removeEventListener("scroll",this.scrollHandler),this.visibilityHandler&&document.removeEventListener("visibilitychange",this.visibilityHandler),this.flushIntervalId&&clearInterval(this.flushIntervalId),v(),C(),E(),this.initialized=!1}}let r=null;const u={async init(e){return r?(console.warn("[HearingSDK] Already initialized. Call destroy() first."),!1):(r=new g(e),r.init())},destroy(){r&&(r.destroy(),r=null)},async flush(){r&&await r.flush()}};return typeof window<"u"&&(window.HearingSDK=u),c.HearingSDK=u,c.InterviewTracker=g,Object.defineProperty(c,Symbol.toStringTag,{value:"Module"}),c}({});
